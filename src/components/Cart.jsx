    ...
    <div className="center mw7 mv4">
      <div className="bg-white pa3 mb3">
          ...
          <tbody>
            {cartItems && cartItems.map((item) => (
              <tr key={item._id}>
                ...
                <td className="tr pv2">
                  <a className="pointer ba b--black-10 pv1 ph2 mr2" onClick={() => updateItemQuantity(item._id, item.quantity - 1)}>-</a>
                  {item.quantity}
                  <a className="pointer ba b--black-10 pv1 ph2 ml2" onClick={() => updateItemQuantity(item._id, item.quantity + 1)}>+</a>
                </td>
        ...
        <div className="tr f4 mv3">
          Total: {getCartTotal ? getCartTotal().toFixed(2) : '0.00'}
        </div>
      </div>
